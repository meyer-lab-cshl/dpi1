...
myFastICA <- function(data, cumulative_proportion = 0.95, seeds = 11:13,
                      hclust_plot = FALSE, verbose = TRUE,
  ...
  n.comp <- myFastICA_estimate_n.comp_by_PCA(data, cumulative_proportion,
                                             n.comp.upper_bound, verbose)
  ...
  number_of_signal_components <- table(sapply(res, function(ica)
    length(ica$signal_components)))
  n.comp.new <- as.integer(names(number_of_signal_components)[
    which.max(number_of_signal_components)])
  while ((n.comp.new < n.comp) & (n.comp.new >= 2)) {
    if (verbose) {
      cat(sprintf("###components: %d -> %d\n", n.comp, n.comp.new))
    }
    n.comp <- n.comp.new
    res <- lapply(
      seeds,
  ...
  if (hclust_plot) {
    ...
    dist <- as.dist(1 - cor(S, method = "spearman",
  ...
  if (verbose) {
    cat(sprintf("#picked seeds: %d\n", seeds[idx]))
  }
  if (verbose) {
    cat(sprintf("#signal component indexes: %s\n",
                paste(res[[idx]]$signal_components, sep = ",", collapse = ",")))
  }
  res[[idx]]
}

myFastICA_estimate_n.comp_by_PCA <- function(X, cumulative_proportion,
                                             n.comp.upper_bound = 100,
                                             verbose = TRUE) {
  pca <- try(prcomp(X, scale = T), TRUE)
  if (class(pca) == "try-error") return(1)

  n.comp <- length(which(summary(pca)$importance[3, ] <
  ...
  if (n.comp > n.comp.upper_bound) {
    n.comp <- n.comp.upper_bound
  }
  if (verbose) {
    cat(sprintf(
...
myFastICA_from_a_seed <- function(data, n.comp, seed = 11, ...) {
  ...
  ica$rescaleS <- sapply(1:n.comp, function(i) {
    apply(ica$S[, i, drop = F] %*% ica$A[i, ], 1, sum)
  })
